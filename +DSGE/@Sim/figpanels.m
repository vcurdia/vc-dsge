...
if isempty(op.PanelList)
    if obj.Model.ObsVar.N>0, op.PanelList{end+1} = 'ObsVar'; end
    op.PanelList{end+1} = 'StateVar';
    if obj.Model.AuxVar.N>0, op.PanelList{end+1} = 'AuxVar';end
end
...
for jL=1:nList
    ...
    nPj = ceil(obj.Model.(Lj).N/nMaxVar);
    for j=1:nPj
        jP = jP+1;
        if nMaxVar==1
            FigPanels(jP).Title = sprintf('%s_%s',Lj,obj.Model.(Lj).Names{j});
            FigPanels(jP).Names = obj.Model.(Lj).Names(j);
            FigPanels(jP).PrettyNames = obj.Model.(Lj).PrettyNames(j);
        else
            if nPj>1
            ...
            FigPanels(jP).Names = obj.Model.(Lj).Names(...
                (j-1)*nMaxVar+1:min(j*nMaxVar,obj.Model.(Lj).N));
            FigPanels(jP).PrettyNames = obj.Model.(Lj).PrettyNames(...
                (j-1)*nMaxVar+1:min(j*nMaxVar,obj.Model.(Lj).N));
        end
